...
public class ThreadCopyWithStop {
    ...
    class SubCopyThread extends Thread {
        ...
        public void run() {
            ...
            try (RandomAccessFile srcFile = new RandomAccessFile(srcPath, "r"); // 读模式，读取文件
                    ...
                    var fos = new FileOutputStream("E:\\schoollearn\\welight\\task5\\" + getName() + "_log.txt");
                    var fis = new FileInputStream("E:\\schoollearn\\welight\\task5\\" + getName() + "_log.txt");
                ...
                while ((readNum = srcFile.read(bytesRead)) != -1) {// 读取文件到结尾（要是会的话
                    ...
                    if (index >= endIndex) {
                        ...
                        oos.close();
                        ois.close();
                        if(new File("E:\\schoollearn\\welight\\task5\\"+getName() + "_log.txt").delete()){
                            System.out.println("成功删除log"+getName());
                        }
                        else{
